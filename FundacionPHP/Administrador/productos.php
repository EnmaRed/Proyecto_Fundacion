...
if (isset($_POST)) {
    if (!empty($_POST)) {
        ...
        if (!empty($nombre) && !empty($edad) && !empty($vacunas) && !empty($descripcion) && !empty($peso_gramos) && !empty($genero) && !empty($estatura) && !empty($raza) && !empty($especie) && !empty($categoria)) {
            $img = isset($_FILES['foto']) ? $_FILES['foto'] : ''; //error en imagen producto de haber borrado linea que configura el tipo de imagen y footo
            ...
            $query = mysqli_query($coneccion, "INSERT INTO productos(nombre, edad, vacunas, descripcion, peso_gramos, genero, estatura, raza, especie, imagen, id_categoria) VALUES ('$nombre', '$edad', '$vacunas' ,'$descripcion','$peso_gramos', '$genero', '$estatura', '$raza', '$especie', '$foto', $categoria)");
...
<div class="d-sm-flex align-items-center justify-content-between mb-4">
    ...
    <a href="#" class="d-none d-sm-inline-block btn btn-sm btn-success shadow-sm" id="abrirProducto"><i class="fas fa-plus fa-sm text-white-50"></i> Agregar Producto</a>
</div>
<div class="row">
    <div class="col-md-12">
